...
#####################################################################
try:
    from retecs.run_experiment_common import *
    from retecs.xcs import XCS
    from retecs.xcs_er import XCS_ER
...
def exp_history_length(iteration):
    ...
    ags = [
        lambda hl: (XCS(2000, list(range(0, 45)), hl), retecs.preprocess_xcs, reward.timerank),
        lambda hl: (XCS_ER(2000, list(range(0, 45)), hl), retecs.preprocess_xcs, reward.timerank),
    ...
    for histlen in history_lengths:
        for get_agent in ags:
            ...
            rl_learning = retecs.PrioLearning(agent=agent,
                                              ...
                                              output_dir=DATA_DIR)
            print("start to train " + agent.name)
